...
private
def refinementTransitionActions
    (p : Project)
    (machine : String)
    (ev : Elem)
    : List Elem
...
private
def concreteStateRelationsAccurate
    (initialization : Bool)
    (variables : List String)
    (actions : List Elem)
    : List Term
...
private
def concreteStateRelationsMode
...
private
def checkedWitnessOrigin
    (component event witnessLabel : String)
