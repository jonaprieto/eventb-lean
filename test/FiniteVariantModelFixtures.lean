...
def modelEventSourceBound
    : CheckedEventSource EventB.Theory.empty
      modelFiniteProject modelFinPO.obligation.component "hold"
    := by
...
def modelVariantSourceBound
    : CheckedVariantSource modelFiniteProject modelFinPO.obligation.component
    := by
  have component : modelFinPO.obligation.component = "M" := by native_decide
  rw [component]
...
theorem modelVarEvaluatorValid
    : modelVarEvaluator.validOnDomain modelVarSource modelVarObligation
    := by
...
theorem modelFinEvaluatorValid
    : modelFormulaModel.validOnDomain modelDomain modelFinObligation
    := by
...
private
def modelFinFormula
    : DomainFormulaAdequacy modelFinPO modelState
...
example
    : ¬ finiteVariantProgress .convergent ([0] : List Int) [0]
    := by
  intro progress
